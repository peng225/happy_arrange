...
void openInputFile(char* inputFileName, ifstream &ifs)
{
  if(inputFileName[0] == 0){
    cerr << "Error: You must specify the input file name by -i option." << endl;
    exit(1);
  }
...
void readData(ifstream &ifs, const int NUM_CHOICES,
	      ...
	      vector<int> &choicesID)
{
  ...
  for(vector<vector<int> >::iterator i = begin(choices);
	...
	cerr << "Error: The value of dept must be between 0 and '# of dept - 1'."
	     << endl;
	exit(1);
      }
